...
import { Bell, Search, User, Settings, Coins, LogOut, ChevronDown } from 'lucide-react';
    ...
    >
      <div className="h-full px-6 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          {/* Coins Display - Use actual user coins */}
          <div className="flex items-center space-x-2 bg-gradient-to-r from-yellow-400/20 to-orange-400/20 px-4 py-2 rounded-2xl border border-white/30 shadow-sm hover:shadow-md transition-all duration-300 backdrop-blur-sm">
            <Coins className="h-5 w-5 text-yellow-600 animate-pulse" />
            <span className="font-bold text-gray-800 text-lg">{user?.cuddleCoins || 0}</span>
            <span className="text-xs text-gray-600 hidden sm:block">Cuddle Coins</span>
          </div>

          <button className="p-3 text-gray-600 hover:text-purple-600 hover:bg-purple-50/80 rounded-2xl transition-all duration-300 relative backdrop-blur-sm">
            <Bell className="h-6 w-6" />
            <div className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full animate-pulse"></div>
          </button>
          
          <button className="p-3 text-gray-600 hover:text-purple-600 hover:bg-purple-50/80 rounded-2xl transition-all duration-300 backdrop-blur-sm">
              ...
              <div className="absolute right-0 mt-2 w-80 bg-white/95 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/40 py-4 z-50 transform transition-all duration-300 scale-100 opacity-100">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center mt-2">
                        <div className="text-sm text-purple-600 font-semibold">Level {user?.level}</div>
                        <div className="mx-2 text-gray-400">•</div>
                        <div className="text-sm text-gray-600">{user?.totalPets} Pets</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="px-6 py-4 border-b border-white/30">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center p-3 bg-gradient-to-r from-yellow-100/60 to-orange-100/60 rounded-2xl">
